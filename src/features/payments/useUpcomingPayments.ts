...
export function useUpcomingPayments(
  ...
  const dividends = useQueries({
    queries: shares.map((p) => ({
      queryKey: ['dividends', p.instrumentUid, window.from],
      queryFn: () => getDividends(p.accounts[0], p.instrumentUid, window.from, window.to),
      staleTime: 12 * HOUR,
    })),
    combine: (rs) => ({ isPending: rs.some((r) => r.isPending), data: rs.map((r) => r.data) }),
  })
  ...
  const payments = useMemo(() => {
    ...
    shares.forEach((p, i) => {
      for (const dividend of dividends.data[i] ?? []) {
        if (dividend.paymentDate) result.push(fromDividend(p, nameOf(p), dividend))
      }
    })

    return result.sort((a, b) => a.date.getTime() - b.date.getTime())
  }, [bonds, shares, coupons.data, dividends.data, instruments, window])
...
function fromDividend(p: Position, name: string, dividend: Dividend): UpcomingPayment {
